...
// Disposable native Play Mode only. GameAudioEvents stays enabled throughout integration.
...
public static class AudioGameplayChecks
{
    ...
    public static IEnumerator Run()
    {
        ...
        // The combined path starts in unsaved DebugRun/Echo, then reaches an idle menu
    ...
    static IEnumerator RunSuite(string suite, string entry)
    {
        ...
        {
            Bind(false);
            Check(SceneManager.GetActiveScene().name == "Main Menu" && events.isActiveAndEnabled
                && Get<WorldManager>(events, "worldManager") == null && Idle(), "menu event component enabled, unbound and intentionally quiet");
            Note("SEPARATE SYNTHETIC SERVICE SUITE: generated native clips, direct calls; NOT game hook evidence");
            yield return AudioServiceChecks.Run();
            Passed += AudioServiceChecks.Passed; Failed += AudioServiceChecks.Failed;
            Note("SEPARATE CATALOG SUITE: configured catalog read-only; temporary catalogs/generated clips for policy; projectile source-only checks");
            yield return AudioCatalogChecks.Run();
            Passed += AudioCatalogChecks.Passed; Failed += AudioCatalogChecks.Failed;
            Check(events.isActiveAndEnabled && Idle(), "menu remains quiet after service/catalog fixture cleanup");
        ...
        Bind(false);
        yield return Wait(.7f);
        Check(SceneManager.GetActiveScene().name == "Main Menu" && Time.timeScale == 1 && Idle()
            && Get<WorldManager>(events, "worldManager") == null && Get<RunStageController>(events, "run") == null,
            "menu cleanup: fresh quiet service, no stale run/world bindings, timeScale restored");
    ...
    static bool Idle() { return audio.GetComponentsInChildren<AudioSource>(true).All(s => s.clip == null && !s.isPlaying); }
