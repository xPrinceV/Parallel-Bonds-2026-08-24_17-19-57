...
Idle
$output = Join-Path $project 'Evidence/RiftLordChecks'
...
$stage = Join-Path $project 'Assets/Game/Editor/RiftLordValidation_Temporary'
...
try {
    ...
    $tools = @('RiftLordChecks.cs','RiftLordChecksBatch.cs')
    New-Item -ItemType Directory (Join-Path $attempt 'executed-tools') | Out-Null
    foreach ($name in ($tools + @('Run-RiftLordChecks.ps1'))) { Copy-Item -LiteralPath (Join-Path $PSScriptRoot $name) -Destination (Join-Path $attempt 'executed-tools') }
    ...
    if (!$PrepareOnly) {
        ...
        $arguments = '-projectPath "{0}" -executeMethod RiftLordChecksBatch.Run -riftSource "{1}" -riftValidationProject "{0}" -riftOutput "{2}" -logFile "{3}"' -f $project,$root,$attempt,$log
        [ordered]@{ boundSeconds=300; utc=[DateTime]::UtcNow.ToString('o'); note='Graphical native editor; no source launch; timeout leaves process untouched.' } |
        ...
        $timeout = !$process.WaitForExit(300000)
