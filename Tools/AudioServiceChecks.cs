...
public sealed class AudioServiceChecks : IDisposable
{
    ...
    IEnumerator Execute()
    {
        ...
        Require(Idle(all) && all.All(s => !s.playOnAwake), "initial sources silent, no clips/playOnAwake");
        ...
        Check(AudioService.Instance == service && Idle(all) && all.All(s => !s.playOnAwake), "OnEnable reacquires singleton and remains silent after frame");
    }
    ...
    public void Dispose()
    {
        ...
        finally { Time.timeScale = savedScale; foreach (var clip in clips) if (clip != null) Object.Destroy(clip); }
    }
}
