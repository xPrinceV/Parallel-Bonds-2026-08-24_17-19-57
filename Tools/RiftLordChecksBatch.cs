...
[InitializeOnLoad]
public static class RiftLordChecksBatch
{
    const string Key = "RiftLordChecksBatch";
    static bool started, finished;
    static EditorWindow gameView;
    static readonly Stack<IEnumerator> stack = new Stack<IEnumerator>();
    static RiftLordChecksBatch() { if (SessionState.GetBool(Key, false)) Hook(); }
    ...
    {
        string project = Path.GetFullPath(Arg("-riftValidationProject")).TrimEnd('/', '\\');
        string source = Path.GetFullPath(Arg("-riftSource")).TrimEnd('/', '\\');
        string allowed = Path.Combine(source, "Library", "DebugRunValidationProject");
        if (!string.Equals(project, allowed, StringComparison.OrdinalIgnoreCase)
        ...
        SessionState.SetString(Key + ".deadline", (EditorApplication.timeSinceStartup + 240).ToString(CultureInfo.InvariantCulture));
        Hook();
        try
        {
            RiftLordChecks.Prefabs();
            foreach (var path in new[] { "Assets/Scenes/Main.unity", "Assets/Scenes/DebugRun.unity" })
            {
                ...
                RiftLordChecks.SceneBinding(scene.GetRootGameObjects().SelectMany(r => r.GetComponentsInChildren<RunStageController>(true)).Single(), scene.name);
            }
            SessionState.SetInt(Key + ".passed", RiftLordChecks.Passed); SessionState.SetInt(Key + ".failed", RiftLordChecks.Failed);
            File.WriteAllLines(Path.Combine(Arg("-riftOutput"), "preflight-evidence.txt"), RiftLordChecks.Evidence);
    ...
    static void Poll()
    {
        ...
        started = true;
        RiftLordChecks.Passed = SessionState.GetInt(Key + ".passed", 0); RiftLordChecks.Failed = SessionState.GetInt(Key + ".failed", 0);
        try
        {
            Focus(); var host = new GameObject("Rift Lord isolated check host"); Object.DontDestroyOnLoad(host);
            stack.Push(RiftLordChecks.Run()); host.AddComponent<RiftLordChecksBatchHost>().StartCoroutine(Drive());
        }
        catch (Exception e) { Finish(2, e.ToString()); }
    }
    static IEnumerator Drive()
    {
        ...
        Finish(0, "Main Victory and DebugRun defeat; both scenes also complete official pre-Start damage, standalone1000 baseline, post-half rings and two real restarts; existing editor prefab branch only");
    }
    ...
    static void Finish(int code, string message)
    {
        ...
        if (code != 0) RiftLordChecks.Check(false, "aborted " + RiftLordChecks.Scenario + ": " + message);
        var summary = new Summary { passed = RiftLordChecks.Passed, failed = RiftLordChecks.Failed,
            ...
            message = message, currentScenario = RiftLordChecks.Scenario, completedScenes = RiftLordChecks.Completed.ToArray(), evidence = RiftLordChecks.Evidence.ToArray() };
        ...
        summary.code = code;
        File.WriteAllText(Path.Combine(Arg("-riftOutput"), "summary.json"), JsonUtility.ToJson(summary, true));
        Debug.Log("RIFT_NATIVE RESULT passed=" + summary.passed + " failed=" + summary.failed + " unexpected=" + summary.unexpectedErrors + " knownSearch=" + summary.knownSearchErrors + " code=" + code);
        SessionState.SetBool(Key, false); EditorApplication.update -= Poll; Application.logMessageReceived -= Log;
        EditorApplication.Exit(code);
    }
}
public sealed class RiftLordChecksBatchHost : MonoBehaviour { }
