...
public sealed class MenuFlowChecks : IDisposable
{
    ...
    IEnumerator Execute()
    {
        ...
        Check(run.IsCompleted && !run.IsDefeated && !run.IsRunning && Time.timeScale == 0
            && !over.gameOverUI.activeSelf, "production boss victory does NOT show GameOver");
    }
    ...
    void BindReady()
    {
        ...
        Check(!over.gameOverUI.activeSelf, "fresh run hides GameOverUI");
    }
    ...
    {
        var matches = InScene<Button>().Where(b => Enumerable.Range(0, b.onClick.GetPersistentEventCount())
        ...
        if (target is GameOverManager gameOver)
            Require(button.transform.IsChildOf(gameOver.gameOverUI.transform), "button belongs to bound GameOver panel");
