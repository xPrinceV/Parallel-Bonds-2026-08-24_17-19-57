...
This version integrates GitHub `main` through `119208b`, including its menu, upgrade-panel and victory-screen design. Use main as the upstream baseline and keep dual-world features as explicit extensions, rather than maintaining another copy of the old scripts.
...
Run `python -B Tools/MainBaselineChecks.py --self-test`, `python -B Tools/MainUiMergeChecks.py --self-test --require-runtime` and `python -B Tools/FontAssetChecks.py --self-test` after integration. These are static gates, not Play Mode acceptance. See [merge notes](Assets/Scenes/MergeNotes.md#main-ui-and-audio-integration-119208b) for deliberate compatibility differences and validation scope.

The main menu plays `MusicMenu` through the same `AudioService` and mixer used in gameplay. Victory and defeat panels are driven by `RunStageController`; their buttons use the normal restart and return-to-menu paths. Legacy AudioManager components and duplicate script classes are not retained.
...
3. Open `Assets/Scenes/Main Menu.unity` (or `Main.unity` to enter gameplay directly).
4. Enter **Play Mode** and select **PLAY**.
